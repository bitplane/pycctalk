...
class CoinAcceptor:
    ...
    def __init__(self, iface, addr=2):
        ...
        self.addr = addr
        self.iface = iface
        self.pollmsg = Message(
            src=1, dst=addr, header=Message.HEADER_READ_BUFFERED_CREDIT
        )
    ...
    def init(self):
        # Get Manufactur
        r = self.iface.send(
            Message(src=1, dst=self.addr, header=Message.HEADER_REQ_MANUFACTURER_ID)
        )
        self.manufacturer = r.data.decode("ascii")

        r = self.iface.send(
            Message(src=1, dst=self.addr, header=Message.HEADER_REQ_EQM_CAT_ID)
        )
        self.equipment_category = r.data.decode("ascii")

        r = self.iface.send(
            Message(src=1, dst=self.addr, header=Message.HEADER_REQ_PRO_CODE)
        )
